...
fn n_cubed_solution(numbers: &Vec<i32>) -> i32 {
    for i in numbers {
        for j in numbers {
            for k in numbers {
                if i + j + k == 2020 {
...
fn main() -> Result<(), std::io::Error> {
    ...
    println!("O(n³): {}", n_cubed_solution(&numbers));
    println!("O(n³/6): {}", better_solution(&numbers));
    println!("rusty O(n³/6): {:?}", rusty_better_solution(&numbers));
